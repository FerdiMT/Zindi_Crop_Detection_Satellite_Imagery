...
# Field size
sizes_fields = df['fid'].value_counts()
sizes_fields = pd.DataFrame(sizes_fields)
sizes_fields.reset_index(drop=False, inplace=True)
sizes_fields.columns = ['fid', 'number_of_pixels']

# We merge them
df_grouped = pd.merge(df_grouped, sizes_fields, left_on='fid', right_on='fid', suffixes=['mean', 'std'])
